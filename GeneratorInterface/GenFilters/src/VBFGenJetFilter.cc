...
//   m_inputTag_GenParticleCollection  = consumes<reco::GenParticleCollection>(edm::InputTag{"genParticles"}))
//   m_inputTag_GenParticleCollection  = consumes<edm::View<reco::GenParticle>>(config.getParameter<edm::InputTag>("genParticles"));
}
  ...
  if(leadJetsNoLepMass) { 
      ...
      while(genJetsWithoutLeptonsP4.size()<2) {
          ...
          for(unsigned int i = 0; i < filGenLep.size(); ++i) {
              ...
              if(dPhi*dPhi + dEta*dEta < deltaRNoLep)
                  jetWhitoutLep = false;
          }
